    ...
    private void AddUpdateSaveOnClick(object? sender, RoutedEventArgs e)
    {
        ...
        if (memberz == null)
        {
            try
            {
                ...
                conn.Open();
                string add = "INSERT INTO members VALUES (" + Convert.ToInt32(ID.Text)+ ", '" + Фамилия.Text + "', '" + Имя.Text + "','" + Convert.ToInt32(Возраст.Text ) +"', '" + Номер_телефона.Text + "', '" + Адрес.Text + "','" + Отряд.Text + "');";
        ...
        else
        {
            try
            {
                conn = new MySqlConnection(connString);
                conn.Open();
                string upd = "UPDATE members SET Surname = '" + Фамилия.Text + "', Name = '" + Имя.Text + "', Age = "+ Convert.ToInt32(Возраст.Text) + ", Phone = "+ Номер_телефона.Text + ", Adress = '"+ Адрес.Text + "', Squad_Name = '"+ Отряд.Text + "' WHERE id = " + Convert.ToInt32(ID.Text) + ";";
